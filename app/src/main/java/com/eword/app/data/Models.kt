 ...
 * 一个词条。
 *
 * 词包 v17 的字段填充情况（10,684 词）：
 *   senses        全部有值，合计 26,068 条（真题 10,865 条、词典补充 15,203 条；
 *                 词包里的取值仍是「大纲补充」，旧包兼容，界面上显示「词典补充」）
 *                 每词条数分布：1 条 2,908 · 2 条 2,685 · 3 条 3,103 · 4 条 1,479 ·
 *                 5 条 490 · 6 条 18 · 7 条 1
 *   examples     10,680 词有，合计 38,024 条（上限 5 条/词）
 ...
 *
 * 4 个词条没有例句、freq 也记为 0（complainers / curbing / emphasise / ordinate）：
 * 前三个在真题里只出现在**题干**或选择题的选项里，卷面没有一句完整用到它的英文、
 * 也没有官方译文；ordinate 只在 `co-ordinate` 里出现过（那是 coordinate 的连字符写法，
 * 不是一个独立的词）。按「核实不了的宁可留空」，例句留空。
 */
