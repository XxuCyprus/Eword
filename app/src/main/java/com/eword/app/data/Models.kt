 ...
 * 一个词条。
 *
 * 词包 v15 的字段填充情况（10,684 词）：
 *   senses        全部有值，合计 12,089 条；其中真题里出现过的 10,862 条，
 *                 词典补充的 1,230 条（标为「大纲补充」）
 *   examples      10,547 词有，合计 37,891 条（上限 5 条/词）
 *   collocations   2,316 词有，合计 3,719 条（只收跨卷出现的搭配）
 *   usage            948 词有（句式模板，只收真题例句里反复出现的）
 *   nearForms      3,812 词有，合计 12,664 条（只收不同族、拼写真的相近的词）
 *   synonyms       6,110 词有，合计 15,225 条（共同义项必须是本词条列出的那条意思）
...
data class WordEntry(
     ...
     *   · 词包自己列的中文意思里有完全相同的词元 —— problem「问题」／question
     *     「问题」／issue「问题，议题」／matter「事情，问题」互为近义。
     *     这条用的是界面上显示的那条意思，所以不会出现「住宿 ↔ 调整」这种错配；
     *     共同词元在两边释义里越靠前，排得越前。
