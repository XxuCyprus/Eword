 ...
 *   examples     13,683 词有，合计 46,820 条（上限 5 条/词）；没有例句的 1,144 词
 ...
 *
 * v22 相比 v21 的五处变化（都在义项、例句、计数上，词表本身没动）：
 *   ① **freq 全表对账**：用唯一成文口径（tools/count_sentence_freq.py）把 14,827 个值
 *      重算一遍，3,838 个词改用重算值（旧值在现网任何口径下都复现不出来，属已不在盘的
 *      旧管线产物）；同时补登 990 个**卷面确实出现过的**变形形态（568 个词，
 *      `absence` 补上 `absences` 后 freq 36 → 58），45 个词按裁决保留旧值 ——
 *      其中 6 个（`calf`/`hoc`/`mise`/`ordination`/`packets`/`starving`）的根因在
 *      切句/切词规则（短句下限、连字符合成词），不在数据侧。
 *   ② **出处写法统一**：173 处写成「英语六级2019年12月第3套」的长写法收成
 *      `2019.12 第3套`，并补回 144 条空着的 level（级别只由 level 字段承担）。
 *   ③ **例句层收尾**：删掉 40 条「同一句收成多个版本」的重复（一条是另一条的子串、
 *      卷面 head+tail 被拆成两条、数字被替换的损坏版）；给 16 个零例句词补上**回卷面
 *      取回完整句**的真句（`rang`/`suite`/`unreal`/`worship`…）；修 5 处句译错配
 *      （`heroism` 两条曾共用同一条错译文、`rags-to-riches` 连字符丢失）。
 *   ④ **释义层收尾**：1,167 条过长释义逐条裁决（精简 496、拆 1、留 670），
 *      并去重 75 条「去掉括注后露出的重复项」（「适应，调整，适应」→「适应，调整」）；
 *      `suite` 补上真题义项「一套，一组」（原只有「套房」「随员」）。
 *   ⑤ **13 条带完形空号的义项支撑**换成卷面真句（`fixed`/`lead`/`makeup`/`narrow`/
 *      `space`/`engaged` 等）；`game` 的动词用法全库只有挖空句，维持原样并记录在案。
 */
