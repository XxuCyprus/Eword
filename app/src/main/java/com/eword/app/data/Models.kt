...
data class Sense(
    ...
    /**
     * 该义项在真题里的**支撑句**（词包 v21 起带）。用户的原始要求是
     * 「这个单词的意思必须有近十年真题里出现过的所有意思，不是词典查来的」——
     * 界面上把原句连同官方译文摆出来，用户才能自己核对，而不是只能信我们。
     *
     * 三个字段一起看：
    ...
    @SerializedName("source") val source: String = "",
    @SerializedName("level") val level: String = "",
    /** 从释义里摘出来的括注（如例句式说明），不参与释义显示，界面上按需用 */
    ...
    /** 是否带真题支撑句（v21 起大部分真题义项都有） */
    val hasSupport: Boolean get() = supportEn.isNotBlank()

    /** 出于哪一套，如「2016.12 第3套」；旧包没有则空 */
    val sourceLabel: String get() = listOf(level, source)
 ...
 *                 v21 起真题义项里 17,971 条带**支撑句**：既有原句也有同卷官方译文
 *                 的 14,539 条（support_kind=译文），只有卷面原句的 3,432 条（=题干）。
 *                 支撑字段就是界面上「这条释义出自哪句真题」那一栏的数据。
 *   examples     13,668 词有，合计 46,846 条（上限 5 条/词）；没有例句的 1,159 词
 ...
 *   ② 每条真题义项带原句 + 同卷官方译文 + 出处，界面上直接摆出来供核对
 *      （字段见 [Sense.supportEn]）。译文只取**同一套卷子**里的那句，避免
 *      「英文一句、中文是隔壁那句译文」。
 *   ③ 例句 42,685 → 46,846 条，且**按义项铺开、年份从新到旧**：每个义项先占一个
